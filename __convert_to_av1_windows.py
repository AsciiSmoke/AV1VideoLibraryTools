...
Usage:
    python __convert_to_av1_windows.py <folder> [options]

Examples:
    python __convert_to_av1_windows.py
    python __convert_to_av1_windows.py "D:\\Videos"
    python __convert_to_av1_windows.py "D:\\Videos" --dry-run
    python __convert_to_av1_windows.py "D:\\Videos" --bitrate-threshold 8000
    python __convert_to_av1_windows.py "D:\\Videos" --handbrake "C:\\Program Files\\HandBrake\\HandBrakeCLI.exe"
"""
...
DEFAULT_HANDBRAKE   = r"C:\Program Files\HandBrake\HandBrakeCLI.exe"
DEFAULT_FFPROBE     = "ffprobe"   # assumed on PATH; override if needed
...
def build_handbrake_cmd(
    ...
    encoder = "nvenc_av1_10bit" if bit_depth >= 10 else "nvenc_av1"
    return [
        handbrake_exe,
        "--input",   str(input_path),
        "--output",  str(output_path),

        # Video — no preset; specify everything explicitly so NVEnc is not overridden
        "--encoder",        encoder,
        "--encoder-preset", "p4",            # balanced NVEnc speed/quality preset
        "--quality",        "28",
        "--vfr",                             # variable framerate, same as source

        # Dimensions: same as source, no scaling or cropping
        "--crop",           "0:0:0:0",
        "--loose-anamorphic",

        # Audio: AAC, preserve channel layout, English only
        "--aencoder",       "av_aac",
        "--mixdown",        "none",
        "--audio-lang-list", "eng",
...
def convert_file(
    ...
    try:
        # stdout/stderr not captured — HandBrake streams directly to the console
        # so you can see the encoder, progress, and any errors in real time.
...
def scan_and_convert(args: argparse.Namespace):
    ...
    json_log_path = scan_folder / f"_av1_conversion_windows_{timestamp}.json"
    text_log_path = scan_folder / f"_av1_conversion_windows_{timestamp}.log" if args.log else None
...
def main():
    ...
    parser.add_argument(
        ...
        metavar="PATH",
        help=f"Path to HandBrakeCLI.exe (default: {DEFAULT_HANDBRAKE!r})",
    )
    parser.add_argument(
        "--ffprobe",
        default=DEFAULT_FFPROBE,
        metavar="PATH",
        help=f"Path to ffprobe.exe (default: '{DEFAULT_FFPROBE}', assumed on PATH)",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
